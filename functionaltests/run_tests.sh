...
if ! timeout ${API_RESPONDING_TIMEOUT} sh -c "until curl --output /dev/null --silent --head --fail http://localhost:8989; do sleep 1; done"; then
    echo "Highlander API failed to respond within ${API_RESPONDING_TIMEOUT} seconds"
    exit 1
fi

echo "Successfully contacted Highlander API"

export BASE=/opt/stack
export HIGHLANDERCLIENT_DIR="$BASE/new/python-highlanderclient"
...
CREDS_FILE=${HIGHLANDERCLIENT_DIR}/functional_creds.conf
cat <<EOF > ${CREDS_FILE}
...
cd $HIGHLANDERCLIENT_DIR

# Run tests
tox -efunctional -- nosetests -sv higlanderclient/tests/functional