...
function run_tests_by_version() {
  export OS_AUTH_URL=""
  echo "$(tput setaf 4)Running integration CLI and python-highlanderclient tests for v$1$(tput sgr 0)"
  export VERSION="v$1"
  nosetests -v highlanderclient/tests/functional/cli/v$1/
  nosetests -v highlanderclient/tests/functional/client/v$1/
  unset VERSION
}
